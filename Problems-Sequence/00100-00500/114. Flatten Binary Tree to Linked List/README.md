...
public:
    ...
    void flatten(TreeNode* root) 
    {
        ...
        // Iterate till the current is alive
        while(current != NULL)
        {
            if(current->left)
            {
                // Make the prev pointer points to the left
                prev= current->left;
                
                // Loop till the prev right exist's
                while(prev->right) prev= prev->right;
                
                // Update the last node of the left subtree to the current right
                prev->right = current->right;
                
                // Update the current right
                current->right= current->left;
    
                // Make the null in the current left
                current->left= NULL;
